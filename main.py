...
def movePiece(blankPiece, row, col):
    if currentPiece[2]:
        print(b[row][col])
        print(b[currentPiece[3]][currentPiece[4]])
        ...
        b[currentPiece[3]][currentPiece[4]].configure(image='', height=4, width=8, command=lambda: movePiece(b[currentPiece[1]][currentPiece[2], currentPiece[3], currentPiece[4]))
# Define button
def button(frame, color, photo, w, h, command):
    b=Button(frame, width=w, height=h, image=photo, bg=color, command=command)
    return b
# CREATE SCREEN
root=Tk()
